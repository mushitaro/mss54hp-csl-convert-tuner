...
| 確認 | 結果 |
|---|---|
| アプリが配信されている | ✅ `RF KORR (TUNED / EXP.)` タブを含めて描画 |
...
| **Worker → D1 の bind** | ✅ 実際に 1 件 POST して確認（下記） |

最後の行はトークンを会話に出さない都合でずっと保留していましたが、
トークンがビルドに入ったことで、認証付きリクエストをブラウザから普通に投げられるようになりました。
検証スクリプト V13 が本番プレビューに対して 1 セッション送り、リモート D1 に

```
id=v13-seeded-session  point_count=3  binaries=70B  app_build=tuner-4ee5ef28152c
```

が入ることを確認済みです（`app_build` はそのビルドの Service Worker キャッシュ名なので、
**どのビルドが書いたか**まで一致します）。確認後、この行は削除してあります（現在 0 件）。
...
| 真の密度効果 | 表の主張の **75 %** | `RF KORR (TUNED / EXP.)` が表を 75 % 側へ下げる（1 回の上限 −0.05 なので数周かかります） |
