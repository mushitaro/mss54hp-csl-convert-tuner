...
**VE と LOW LOAD は 1 行になりました（`ALPHA-N`）。** 設計時はこの 2 つを別トグルにしていて、
上の箱もそう書いていました。`kf_rf_soll` は 24×20 の**表 1 枚**で、TI_F_STAT が補正から外れ
`ltft` が両方に入って以降、2 つは `Old × stft × ltft × rf_korr` という**同じ式**になり、違うのは
升ごとに要求する証拠の量だけになりました。1 枚の表に 2 つのトグルを置くと、算術がもう提示して
いない選択を読者に迫ることになり、しかも片方だけ武装できてしまう —— **半分がドライブから、
半分が BASE から書かれた表**で、それを画面上で言うものが何も無い状態です。

帯の区別は**行の中に残ります**。据え切りアイドルは 1 つの升に数分留まれるのに対しスイープは
一瞬で横切るので、2 つの帯は違うバーを必要とします。だから帯ごとの報告は続きますが、
**訊かれる決定は 1 つ**になりました。`SHAPE` はその上のモードで、独立した表ではありません。
