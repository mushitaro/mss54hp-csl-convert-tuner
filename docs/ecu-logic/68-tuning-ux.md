...
# 第 1 部 — `kf_rf_soll` は 1 枚の表で、導出も 1 本である

## 1.1 1 枚の表と、その中の領域

`kf_rf_soll`（`0xD356`、24 行 × 20 列）は **1 枚しかない**。導出も 1 本で、
**24 行すべてが同じ式・同じ証拠ゲートで書かれる**。行ごとに違うのは規則ではなく、
**そこへどういう運転で到達するか**である。
...
├──────────────────────────────────────────────┤
│ 行 1–12  0.146 – 3.198 %    アイドル・超低負荷              │
├──────────────────────────────────────────────┤
│ 行 13–19  5.0 – 30.0 %      巡航・中負荷                    │
├──────────────────────────────────────────────┤
...
**1 升を書く条件は、その升が何行目でも変わらない。**

| | 全 24 行に同じく |
|---|---|
| 補正式 | `トリム（STFT × LTFT）× rf_korr` |
| 必要サンプル数 | **3**（1 升あたり。`VE METHOD` = DIRECT の既定） |
| 必要な重み | **2.5**（双一次の重み和） |
| セルの幅 | 行で違う —— 低開度行は 0.05 % 開度、中負荷行は 5 % 開度 |

**同じバーを満たすのに要る運転は、行によって違う。**
`la_f_regler` は 1–2 Hz のリミットサイクルを持ち、DS2 のサンプルレートは 3–5 Hz。
据え切りアイドルは 1 つの升に数分留まれるので、30 サンプル取っても「1 周期を 30 点で見た」
だけかもしれない。信号待ち 1 回は 1 観測であって 30 観測ではない。だから低開度行は
**同じ状態を長く保持するのではなく、日や状態を変えて何度も通る**ことで埋める。

（第 3 部以降の画面例に出てくる「30 サンプル・3 訪問」は、`67-tuning-procedure.md` 第 3.1 節が
**運転の目標**として置いている数であって、ツールの採択条件ではない。）

## 1.3 したがってユーザーは「モード」を選ばない

**ユーザーがすることは、走ることだけである。**
1 回の走行には、アイドルのサンプルも、超低負荷のサンプルも、巡航のサンプルも入っている。
ツールは各サンプルの `aq_rel_rf` を計算し、**どの升に落ちるかだけを見る。**
...
**タブは、同じ 1 枚の表の違う“見え方”である。**
どれを開いていても、走っているログは 1 本、集まっている証拠は 1 組である。
...
## 2.2 層 2 の中では、どの升も同じ 1 本の走行から埋まる

1 回の走行が、アイドルの升にも巡航の升にも証拠を供給する。**判定は升ごとに独立**なので、
基準を満たした升だけが書かれ、満たさなかった升は BASE のまま残る。
...
**規則の境界線は引かない。** 24 行は同じ式・同じ証拠ゲートで書かれるので、
行 12（3.198 %）と行 13（5.005 %）の間に線を引くと「ここから先は別の道具が書いた」という
読み違いを招く。色は補正の向きに使い切っているので、升が何で止まっているかはタップで出す。
...
ハブの WRITE メニューは、**各項目の可否と理由**を出す。出荷時の姿:
...
WRITE
  ...
  ☑ SHAPE                 3 cells
  ☑ WARMUP                derived
  ☐ RF KORR               証拠なし（V > 20 km/h の走行が必要）   ← preview のみ
    IDLE            seal                                        ← preview のみ
    INERTIA         —     proposal only                         ← preview のみ
```

トグルを入れると、ハブの WRITE という語の下に項目名が並ぶ。開かなくても
「次の書き込みに何が入るか」が読めるようにするためです。

**`kf_rf_soll` のトグルは `ALPHA-N` の 1 行だけです。** 設計時は低開度行と中負荷行を
別トグルにしていて、上の箱もそう書いていました。`kf_rf_soll` は 24×20 の**表 1 枚**で、
TI_F_STAT が補正から外れ `ltft` が入って以降、どの行も `Old × stft × ltft × rf_korr` という
**同じ式**で導かれます。1 枚の表に 2 つのトグルを置くと、算術がもう提示していない選択を
読者に迫ることになり、しかも片方だけ武装できてしまう —— **半分がドライブから、半分が BASE
から書かれた表**で、それを画面上で言うものが何も無い状態です。**2 つ目のトグルを足しては
いけない理由がこれです。**

いまは導出も 1 本、証拠ゲートも全升で 1 組です。据え切りアイドルは 1 つの升に数分留まれるのに
対しスイープは一瞬で横切るので、**同じバーを満たすのに要る運転は行ごとに違います**。
報告はその差が読めるように続きますが、**訊かれる決定は 1 つ**です。
`SHAPE` はその上のモードで、独立した表ではありません。

所有権の規則自体は変わっていません —— `composeVeGrid` が升ごとに所有者を決めます:

```
導出が採択した升        → 導出の値
SHAPE が修復した升      → SHAPE の値（測定のある升は動かさない）
どちらも触っていない升  → BASE のまま
全部 OFF                → 表に 1 バイトも触れない
```
...
| | アイドル 3 種（§3.4 の ①） | 10 分 | 行 3–6 が埋まる |
| | 超低負荷 + 中負荷の巡航 | 40 分 | 低開度行と中負荷行が同時に埋まる |
| | WRITE（ALPHA-N）→ フラッシュ | 20 分 | 新キャンペーン |
...
**境界での衝突は無い。** 導出は 1 本なので、行 12（3.198 %）の升も行 13（5.0 %）の升も
同じ式・同じゲートを通る。かつてはここに継ぎ目があり、上下を別の導出が担当していた ——
継ぎ目ごと無くなった。1 サンプルは双一次で 4 つの升に配られるが、採択は升ごとに独立している。

```
if (導出がこの升を採択した)        → 導出の値
else if (SHAPE がこの升を修復した) → SHAPE の値
else                               → BASE のまま
```
...
（低開度行 3 升・それより上 8 升、走行 4 本ぶん）

  ▸ 先に ALPHA-N を書いてから、空気側に進む
...
## 5.3 部分適用 — 基準を満たした升だけが書かれる

同じ走行で、巡航の升は基準を満たし低開度の升は満たさない（あるいはその逆）は普通に起きる。
**書かれるのは満たした升だけ**で、落ちた升は理由が出る:

```
ALPHA-N       行 3 は採択 0 升（サンプルは 84 ある）
...
- 全升に同じ 1 組の証拠ゲート（サンプル数・重み。STATISTICAL では自己シェア・独立性・散らばり・有意性）
...
| **カバレッジ・マップ** | 升の状態（採択・不足・触らない）を 1 枚で読む画面が無い。**1 枚に統合する**のがこの UX の中心 |
...
| ~~**アイドル行の解錠**~~ | **解消済み。** 補正式から `TI_F_STAT` が外れ、`requireTiBranchProven` も既定 OFF になり、その後この行を別に導出していた経路ごと無くなった。いまアイドル行の升を止めているのは、他の升とまったく同じ証拠ゲート（サンプル数・重み）だけである |
