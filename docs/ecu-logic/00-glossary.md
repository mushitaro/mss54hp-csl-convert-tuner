...
| `90-sources.md` | 出典対応表 |
| `91-evidence.md` | **根拠の等級と、条件についての一つの規則** |
...
DS2 の応答は専用の割り込みではなく、**19.53 ms 周期の通信タスク**から組み立てられる。
PIT（プログラマブル・インターバル・タイマ）の取り分は CPU の **2.34 %**。

このツールの計測に効くのは次の一点である: **ECU の「思考時間」は連続量ではなく、
この 19.53 ms で量子化されている。** セッション #904 の per-exchange トレースで block 3 の
ターンアラウンドが 13〜85 ms に散らばるのは、要求がタスク周期のどの位相で届いたかの差であって、
負荷でも回線品質でもない。中央値（約 40 ms）はおおむね周期の半分 + 処理時間にあたる。

block 19 だけが 85〜100 ms と別の分布を持つのは位相ではなく、`la_f_regler` が**スレーブ側の信号**
であり、マスタがスレーブのオーバーレイを待つためである。これが、そのブロックを 1/8 レーンに
落として同じ値を master RAM（`0xFF80CA`）から読む理由になっている
（→ `95-faster-logging.md`、`src/lib/dme-link/ramMap.ts`）。
...
- 等級の付け方と引用の書式は `91-evidence.md`。規則は一つだけ:
  **条件・分岐についての主張は逆コンパイル原文 `[C]` を引く。**
  解析結果 `[G]` は「何に触れるか」には引けるが、「その条件が存在するか」には引けない。
  `npm run verify:cal-docs` が引用先の実在を機械的に確かめる（正しさは確かめない）。
