...
check('2563 ram symbols', nodeCounts.ram === 2563, `got ${nodeCounts.ram}`);

check('71 categories', graph.categories.length === 71, `got ${graph.categories.length}`);

check('27642 edges', graph.edges.length === 27642, `got ${graph.edges.length}`);
...
check('1384 funcs carry recovered formulas', withStmts === 1384, `got ${withStmts}`);
...
const expectLocked = [
    ...
    // Joined the app-managed set with the RF GATE patch: buildPatchedBuffer writes this curve in
    // BOTH directions on every build, so a CALIBRATION edit here would be overwritten silently.
    ['kl_rf_korr_rf_min', 'app-managed'],
