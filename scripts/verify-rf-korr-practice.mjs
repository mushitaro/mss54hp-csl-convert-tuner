 ...
 *
 * Any BASE the app can load will do; scripts/verify-rf-korr.mjs's session directory has one inside
 * binaries.json if you have no loose file.
 */
...
for (let i = 0; i < seconds * HZ; i++) {
    const s = drive.sample(i / HZ);
    points.push({
        time: s.time, rpm: s.rpm, rawLoad: s.rawLoad,
        stft1: s.stft1, stft2: s.stft2, lambda1: s.stft1, lambda2: s.stft2,
        coolantTemp: s.coolantTemp, rf: s.rf, exhaustTemp: s.exhaustTemp,
    });
}
...
const annotated = calc.annotateRfKorr(veMap, processed.rfKorrData, egt);
