 ...
 * its first fourteen rows at or below the top of the block SHAPE works on — so it is derived almost
 * entirely from the rows the repair exists to fix. It used to be generated once, from the tuned map,
 * at the moment the VE calculation finished, and never revisited: applying a repair changed the main
 * table in the flash and left this one interpolated from the unrepaired grid, with the WARMUP tab
 * rendering that third answer.
 *
...
/**
 * The last opening row `repairLowLoadShape` touches: row 12, 3.20 %.
 *
 * It used to be imported, back when a second derivation happened to end at the same row and the two
 * shared a constant. That derivation is gone; the repair's own block is not, so the row is stated
 * here, in the only file that still passes it.
 */
const SHAPE_TOP_ROW = 12;
...
// A recorded session from the developer's own car, left off the public preview branch
// (NOT_FOR_PUBLIC in scripts/release-scope.mjs). The grid it tunes is the subject of every check.
const FIXTURE = 'scripts/fixtures/session-920-base.bin';
if (!existsSync(FIXTURE)) {
    console.log(`\n  SKIP  ${FIXTURE} is absent — there is no tuned grid to derive the warmup table from.`);
    process.exit(0);
}
...
const repair = repairLowLoadShape(tuned.data, APP_CONFIG.MSS54HP.AXIS_LOAD, weight, SHAPE_TOP_ROW);
...
// WARMUP tab now make.
const off = writtenVeGrid(tuned.data, null);
const on = writtenVeGrid(tuned.data, shapeArm);
...
check('SHAPE armed without a composition contributes nothing',
    writtenVeGrid(null, shapeArm) === null);
...
// Why the effect is this large: the cold table is mostly derived from the repaired rows.
const below = CSL_STOCK_WARMUP_LOAD.filter(y => y <= APP_CONFIG.MSS54HP.AXIS_LOAD[SHAPE_TOP_ROW]).length;
console.log(`  (${below} of ${CSL_STOCK_WARMUP_LOAD.length} warmup rows sit at or below `
    + `${APP_CONFIG.MSS54HP.AXIS_LOAD[SHAPE_TOP_ROW]} % opening, the top of the repaired block)\n`);
check('the warmup axis really does live in the repaired rows', below >= 10, `${below} rows`);
