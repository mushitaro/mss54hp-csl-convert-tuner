...
/** Nearest writable value for a scaled field, plus how far away it was. */
export function quantise(def: EcuNumericDef, physical: number): { value: number; exact: boolean; step: number } {
    ...
    const raw = Math.max(min, Math.min(max, Math.round(rawExact)));
    const value = def.scaling.toPhysical(raw);

    // Measured, not assumed. A reciprocal scaling (`5.12/x`) has a step that varies by two orders
    // of magnitude across its range, so a single constant would be a lie at one end or the other.
    const neighbour = def.scaling.toPhysical(Math.max(min, Math.min(max, raw + 1)));
    const step = Math.abs(neighbour - value);

    return { value, exact: Math.abs(value - physical) < 1e-9, step };
}
