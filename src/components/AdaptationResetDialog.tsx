...
export const AdaptationResetDialog: React.FC<Props> = ({ onRead, onReset, onClose, onResetComplete, error, errorKind }) => {
    ...
    // The lint here is a false positive that is worth keeping suppressed rather than worked around:
    // `load` is async and its first statement is `await onRead()`, so nothing it does is synchronous
    // with this effect. The rule cannot see through the call. Restructuring to satisfy it would mean
    // moving an await out of a function whose whole shape is "read the DME, then show what it said".
